...
内置文件加密算法（采用高安全性设置，性能可能不高），也可以通过运用内核进行自行定制。
...
部分函数的基本用法会被忽略或者完全无法使用，请看头文件中的定义体，我们会抽空更改README文件。
...
`decrypt`能够让你对字符串进行解密。

## encrypt

`encrypt`能够让你对字符串进行加密，并且以二进制进行存储。具有区域性划分，破解难度极大。
...
`makeencryptfile`能够让你创建一个文件的加密文件。请注意，在这个函数中，目标文件名是在原来的文件名后方加上加密后缀。

## makedecryptfile

`makedecryptfile`能够让你回复一个加密文件的原文件。请注意，在这个函数中，目标文件名是去掉了加密后缀的文件名，如果你不知道原文件是什么格式，请使用文件头进行判定，本函数不实现。
